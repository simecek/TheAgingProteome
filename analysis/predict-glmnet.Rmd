...
for (c in cols) {
  new.line <- data.frame(variable = c,
                         se = sqrt(mean((pred.both[,c] - pred.both[,"Age"])^2)),
...
for (c in cols) {
  new.line <- data.frame(variable = c,
                         se = sqrt(mean((pred.both[,c] - pred.both[,"Sex"])^2)),
...
ggplot(pred.both, aes(x=Age.mrna_kidney, y= Age.mrna_heart, color=factor(Age), shape=factor(Sex))) +
  geom_point() +
  labs(title = "Age / mRNA", 
...
ggplot(pred.both, aes(x=Age.prot_kidney, y= Age.prot_heart, color=factor(Age), shape=factor(Sex))) +
  geom_point() +
  labs(title = "Age / protein", 
       subtitle = paste("p-value", pval.output(pval))) +
  theme(legend.position="none")
```
...
ggplot(pred.both, aes(x=Sex.mrna_kidney, y= Sex.mrna_heart, color=factor(Sex), shape=factor(Age))) +
  geom_point() +
  labs(title = "Sex / mRNA", 
       subtitle = paste("p-value", pval.output(pval))) +
  theme(legend.position="none")
...
ggplot(pred.both, aes(x=Sex.prot_kidney, y= Sex.prot_heart, color=factor(Sex), shape=factor(Age))) +
  geom_point() +
  labs(title = "Sex / protein", 
       subtitle = paste("p-value", pval.output(pval))) +
  theme(legend.position="none")
```
